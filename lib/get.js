...
const getByHanzi = db => query => new Promise((resolve, reject) => {
...
const getListByPinyin = db => query => new Promise((resolve, reject) => {
  db.get(query, (err, data) => {
    if (err) reject(err)
    else resolve(data)
  })
...
const getByPinyin = (hanziDB, pinyinDB) => {
  ...
  const listByPinyin = getListByPinyin(pinyinDB)
